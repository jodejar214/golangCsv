...
func readInputFile() []string {
    ...
    log.Println("Need to retrieve", len(urlList), "csv files.")
    return urlList
}
...
func getMedianAgeAndName(countMap map[int]int, nameMap map[int]string, ageList []int) {
	...
	if totalCount % 2 == 1 {
		...
		log.Println("Median Index High:",medIndHigh)
		...
		log.Println(ageList[medIndHigh])
		log.Println("The median high is:", medAgeHigh)
		log.Println((ageList[medIndLow] + ageList[medIndHigh])/2.0)
